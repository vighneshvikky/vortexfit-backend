...
import { IMessageRepository, MESSAGE_REPOSITORY } from '../../repository/interface/messages.repository.interface';
...
constructor(
  @Inject(MESSAGE_REPOSITORY) private readonly _messageRepository: IMessageRepository,
  ...
  async saveMessage(
    ...
    const message = await this._messageRepository.saveMessage(senderId, receiverId, content);
  ...
  async getHistory(
    ...
    const messages = await this._messageRepository.getHistory(roomId, skip, limit);
